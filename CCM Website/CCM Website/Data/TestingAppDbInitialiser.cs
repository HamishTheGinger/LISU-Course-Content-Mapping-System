        ...
        public static void TestingSeed(IApplicationBuilder applicationBuilder)
        {
            using (var serviceScope = applicationBuilder.ApplicationServices.CreateScope())
            {
                ...
                if (!context.WeekActivities.Any())
                {
                    ...
                    var Remote = context.TaskLocation.First(tl => tl.LocationName == "Remote (Home)");
                    
                    //Task Status
                    var Completed = context.TaskProgressStatus.First(ts => ts.StatusName == "Completed");
                    var Progress = context.TaskProgressStatus.First(ts => ts.StatusName == "In-Progress");
                    var Unassigned = context.TaskProgressStatus.First(ts => ts.StatusName == "Unassigned");
